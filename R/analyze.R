...
           Covs,  slopeprior, nChains = 3, conv.limit = 1.05, niters = 50000, nruns = 5000, setsize = 4000, alphaprior, 
		   beta.norm.prior, beta.ord.prior, c1prior, dcprior, varprior, varprior.params, path = "", i, insufficient_data)
  {
...
    names(score.range) = outnames
	...
	if (insufficient_data = TRUE) {
		Pain.out = Fatigue.out = Drowsy.out = Sleep.out = Thinking.out = Constipation.out = Neuropain.out = list("DIC" = 1000)
	}

	if (i != 4 && insufficient_data == FALSE) {
	...
	} else if (i == 4 && insufficient_data == FALSE) {
		cat("\n \n \n Pain", i, ".1 \n \n \n")

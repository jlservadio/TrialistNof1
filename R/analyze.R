analyze <-
  function(Pain, Fatigue, Drowsy, Sleep, Thinking, Constipation, Treat, Neuropain, score.range = c(30, 4, 5, 4, 4, 4,30),
	...
	beta.norm.prior, beta.ord.prior, c1prior, dcprior, varprior, varprior.params, path = "", i, insuf)
{
	...
	Pain.out = tryCatch({run.norm(Pain, Treat, Covs, "normal", "hom", nChains, conv.limit, niters, nruns, setsize, 
		slopeprior, alphaprior, beta.norm.prior, varprior,varprior.params, path = "", insuf)}, error=function(cond){
	...
	Fatigue.out = tryCatch({run.ord(Fatigue, Treat, Covs, score.range[2]+1, "cumlogit", nChains, conv.limit, niters, nruns,
		setsize, beta.ord.prior, dcprior, c1prior, path, insuf)}, error=function(cond){caught <- list("DIC" = 1000)})
	...
	Constipation.out = tryCatch({run.ord(Constipation, Treat, Covs, score.range[6]+1, "cumlogit", nChains, conv.limit, niters,
		nruns, setsize, beta.ord.prior, dcprior, c1prior, path, insuf)}, error=function(cond){caught <- list("DIC" = 1000)})
    ...
    for(i in 1:nrow(change.interval)){
		output[[i]] = list("interval" = structure(as.list(change.interval[i,]), names=c("P025", "Median", "P975")),
			"probs" = structure(as.list(change.probs[i,]), names=paste("Proportion",c("< -0.2", "-0.2 - 0", "0 - 0.2", 
			"> 0.2"))))
    }

	output[[1 + nrow(change.interval)]] = list("Pain.DIC" = Pain.out[["DIC"]], "Fatigue.DIC" = Fatigue.out[["DIC"]], 
		"Drowsy.DIC" = Drowsy.out[["DIC"]], "Sleep.DIC" = Sleep.out[["DIC"]], "Thinking.DIC" = Thinking.out[["DIC"]],
		"Constipation.DIC" = Constipation.out[["DIC"]], "Neuropain.DIC" = Neuropain.out[["DIC"]])
